...
public class LiteraAccessor implements TweenAccessor<Litera> {
	...
	public int getValues(Litera target, int tweenType, float[] returnValues) {
		switch (tweenType) {
			case POS_XY:
				returnValues[0] = target.getX();
				returnValues[1] = target.getY();
				return 2;
			case POS_Y : returnValues[0] = target.getY();
				return 1;
	...
	public void setValues(Litera target, int tweenType, float[] newValues) {
		switch (tweenType) {
			case POS_XY: target.setPosition(newValues[0], newValues[1]); break;
			case POS_Y: target.setY(newValues[0]); break;
